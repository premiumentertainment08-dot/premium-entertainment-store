...
function filterCat(c){cat=c;document.addEventListener('DOMContentLoaded',()=>render());scrollTo({top:document.querySelector('main').offsetTop-70,behavior:'smooth'})}function filterProducts(){render()}
...
function payNow(){if(!selPlan)return;let o=selPlan[5][selPlan._selectedIndex||0];if(!o||!o[1]){waOrder();return}let amount=o[1]*qty;document.getElementById('paymentBox').style.display='block';document.getElementById('qrAmount').textContent='₹'+amount;document.getElementById('paymentBox').scrollIntoView({behavior:'smooth',block:'center'})} function copyUPI(){navigator.clipboard?.writeText('Q00403870@ybl').then(()=>alert('UPI ID copied')).catch(()=>alert('UPI ID: Q00403870@ybl'))}
